...
# DATABASE STRUCTURES
# For Heroku, postgesql is used, and only one database is allowed
# We must then use one database with 5 events * 2 tables per event = 10 tables
# We can then iterate over each table to find the right data matching to dbName value
...
# GLOBAL VARIABLES
dbName = "ms" # Default to ms
# HEROKU DATABASES
databases = {
    "ms" : "DATABASE_URL",
    "ws" : "HEROKU_POSTGRESQL_CRIMSON_URL",
    "md" : "HEROKU_POSTGRESQL_WHITE_URL",
    "wd" : "HEROKU_POSTGRESQL_MAUVE_URL",
    "xd" : "HEROKU_POSTGRESQL_AMBER_URL",
}
...
def selectDB():
    ...
    app.config["SQLALCHEMY_DATABASE_URI"] = os.environ[databases[dbName]]
    return redirect("/")
